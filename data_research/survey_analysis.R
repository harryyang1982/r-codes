...
survey_sum <- survey %>% 
  ...
  mutate(q5 = fct_recode(q5,
                         "지방선거 모든 분야에서 \n정당 공천을 하는 것이 바람직함" = "1",
                         "광역자치단체장 및 의원 선거에선 정당 공천을 하되, \n기초자치단체장 및 의원 선거에선 공천 않고 \n정당 표기 하지 않는 선거가 바람직함" = "2",
                         "지방선거 모든 분야에서 정당 공천과 정당 표기 없이 \n선거를 하는 것이 바람직함" = "3",
                         "잘 모르겠음" = "4")) %>% 
  mutate(q6 = fct_recode(q6,
                         "광역의원 및 기초의원 모두 비례대표 비중을 \n늘리는 것이 바람직함" = "1",
                         "광역의원에 대해서만 비례대표 비중을 \n늘리는 것이 바람직함" = "2",
                         "지역의원에 대해서만 비례대표 비중을 \n늘리는 것이 바람직함" = "3",
                         "지방의회에서 정당 비례대표제를 강화하는 것을\n 찬성하지 않음" = "4",
                         "잘 모르겠음" = "5")) %>% 
  mutate(q7 = fct_recode(q7,
                         "교육감 직선제 유지하되, \n교육감 권한 강화" = "1",
                         "교육감 직선제 유지하되, \n광역자치단체장과 함께 선출하는 러닝메이트제로 전환" = "2",
                         "교육감 직선제 유지하되, \n교육감이 광역자치단체장과 충돌하지 않도록 권한 축소 조정" = "3",
                         "교육감 직선제는 폐지하는 것이 \n바람직함" = "4",
                         "잘 모르겠음" = "5")) %>% 
...
for (i in seq_along(mapp)) {
  plots[[i]] <- ggplot(data=survey_sum, aes_string(x=mapp[[i]][["xer"]], fill=mapp[[i]][["filler"]])) + 
    ...
    coord_flip() + 
    guides(fill = guide_legend(ncol=2)) + theme(legend.position = "bottom", legend.direction = "horizontal", legend.title = element_blank())
}
